...
def test_reserve_vehicle_successfull(mocker):
    ...
    mocker.patch('run.check_available', return_value=True)
    mocker.patch('api.Calendar.schedule_event', return_value={"SUCCESS" : "Successfully created an event"})
    mocker.patch('run.send_ephemeral_message', return_value='SUCCESS')
...
def test_reserve_vehicle_error_schedule(mocker):
    ...
    mocker.patch('run.send_ephemeral_message', return_value='')
    res = reserve_vehicle(test_payload, 'test')
...
def test_reserve_vehicle_not_available(mocker):
    ...
    mocker.patch('api.db.index.get_vehicle_by_name', return_value=Vehicle())
    mocker.patch('run.check_available', return_value=False)
    mocker.patch('run.send_ephemeral_message', return_value='')
    res = reserve_vehicle(test_payload, 'test')
...
def test_reserve_vehicle_exception(mocker):
    ...
    mocker.patch('api.db.index.get_vehicle_by_name', return_value=Vehicle())
    mocker.patch('run.check_available', side_effect=Exception("mocked error"))
    mocker.patch('run.send_ephemeral_message', return_value='')
    res = reserve_vehicle(test_payload, 'test')
    assert res['status'] == 500
...
def test_reserve_vehicle_no_date_time(mocker):
    mocker.patch('api.db.index.get_vehicle_by_name', return_value=Vehicle())
    mocker.patch('run.send_ephemeral_message', return_value='')
    mocker.patch('run.get_start_end_time_from_payload', return_value=('NoneTNone', 'NoneTNone'))
    res = reserve_vehicle(test_payload, 'test')
    assert res['status'] == 400
...
def test_reserve_vehicle_missing_time(mocker):
    mocker.patch('api.db.index.get_vehicle_by_name', return_value=Vehicle())
    mocker.patch('run.send_ephemeral_message', return_value='')
    mocker.patch('run.get_start_end_time_from_payload', return_value=('2022-08-10TNone', '2022-08-10TNone'))
    res = reserve_vehicle(test_payload, 'test')
    assert res['status'] == 400
...
def test_reserve_vehicle_missing_date(mocker):
    mocker.patch('api.db.index.get_vehicle_by_name', return_value=Vehicle())
    mocker.patch('run.send_ephemeral_message', return_value='')
    mocker.patch('run.get_start_end_time_from_payload', return_value=('NoneT13:00', 'NoneT14:00'))
    res = reserve_vehicle(test_payload, 'test')
    assert res['status'] == 400
...
def test_check_vehicle_exception(mocker):
    mocker.patch('api.db.index.get_vehicle_by_name', return_value=Vehicle())
    mocker.patch('run.check_available', side_effect=Exception('mocked exception'))
    mocker.patch('run.send_ephemeral_message', return_value='')
    res = check_vehicle(test_payload, 'test')
    assert res['status'] == 500
...
def test_find_similar_commands_no_similar():
    response = find_similar_commands('asdf')
